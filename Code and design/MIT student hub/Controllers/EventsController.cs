...
namespace MIT_student_hub.Controllers
{
    public class EventsController : Controller
    {
        ...
        public EventsController(IConfiguration iconfiguration)
        {
        ...
        public IActionResult Index()
        {
            ...
            if (!string.IsNullOrEmpty(user))
            {
                ...
                {
                    con.Open();
                    SqlCommand JCmd = new SqlCommand("GetEventsByUser", con);
                    JCmd.CommandType = CommandType.StoredProcedure;
                    JCmd.Parameters.Add(new SqlParameter("@Userid", user));
                    JCmd.Parameters.Add(new SqlParameter("@Posttype", 'E'));
                    ...
                    List<UserDataModel> listob = GetAllEventList();
                    ViewData["AllEventlist"] = Convert.ToString(listob[0].MessageString);
                }
                catch (Exception ex)
                {
        ...
        public ActionResult PostForm(PostModel postvalues)
        {
            ...
            if (!string.IsNullOrEmpty(userName))
            {
                ...
                {
                    con.Open();
                    SqlCommand dCmd = new SqlCommand("Update_Event_Post", con);
        ...
        {
            List<UserDataModel> ListallEvents = GetAllEventList();
            return View(ListallEvents);
        }

        public List<UserDataModel> GetAllEventList()
        {
            SqlDataReader sqdr;
            List<UserDataModel> PostModelList = new List<UserDataModel>();
            SqlConnection con = new SqlConnection(ConnectionString);
            ...
            {
                con.Open();
                SqlCommand JCmdlist = new SqlCommand("GetAllEventPost", con);
